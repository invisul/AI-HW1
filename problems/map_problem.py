...
class MapProblem(GraphProblem):
    ...
    def expand_state_with_costs(self, state_to_expand: GraphProblemState) -> Iterator[OperatorResult]:
        ...
        for link in junction.outgoing_links:
            neighbour = MapState(junction_id=link.target)
            if neighbour is None:
                continue
            yield OperatorResult(successor_state=neighbour, operator_cost=link.distance)
    ...
    def is_goal(self, state: GraphProblemState) -> bool:
        ...
        # DONE [Ex.10]: modify the returned value to indicate whether `state` is a final state.
